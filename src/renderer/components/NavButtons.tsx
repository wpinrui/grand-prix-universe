...
const SECTION_SELECTED = 'bg-blue-600 text-white';
...
export function SectionButton({ section, isSelected, onClick }: SectionButtonProps) {
  ...
  const className = `${SECTION_BASE_STYLES} ${isSelected ? SECTION_SELECTED : SECTION_UNSELECTED}`;
...
const SUBNAV_SELECTED = 'bg-blue-600 text-white';
...
export function SubNavButton({ subItem, isSelected, onClick }: SubNavButtonProps) {
  ...
  const className = `${SUBNAV_BASE_STYLES} ${isSelected ? SUBNAV_SELECTED : SUBNAV_UNSELECTED}`;
